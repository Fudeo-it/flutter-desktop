...
class MainPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
        body: BlocConsumer<TimerBloc, TimerState>(
          listener: (context, state) {
            _updateSystemTray(context, state);

            if (state is CompletedTimerState) {
  ...
  void _updateSystemTray(BuildContext context, TimerState state) {
    final appWindow = context.read<AppWindow>();
    final systemTray = context.read<SystemTray>();

    final completed = state is CompletedTimerState;

    App.updateSystemTray(
      appWindow,
      systemTray,
      onReset: completed ? context.read<TimerBloc>().start : null,
    );
  }
}
